...
1) Manual: generate `docs/index.html` locally and push to the `main` branch. GitHub Pages can serve the `docs/` folder.
   ...
   - Commit and push `docs/index.html` to `main` (or gh-pages branch) and enable Pages in repo Settings > Pages. If you are using the **manual** approach, set the source to `main`/`docs`; otherwise for the automatic workflow choose **gh-pages** branch (root). The first visit may still show old content until GitHub finishes building and caches expire.
...
2) Automatic: use the provided GitHub Action `publish-pages.yml` which runs on push and deploys `docs/` to Pages.
   ...
   - In GitHub repo settings, ensure GitHub Pages is enabled for the repository and that its source is set to the **gh-pages** branch (after the workflow runs you can switch it here if necessary).
